...
| Pre-registration | POST | `/api/v1/pre-registrations` | Submit landing pre-registration, optional survey/interview answers, and contact consent | Public |
| Pre-registration | GET | `/api/v1/admin/pre-registrations` | List contact-bearing pre-registration records | Admin/Ops |
| Pre-registration | PATCH | `/api/v1/admin/pre-registrations/{registrationId}` | Update follow-up and benefit confirmation status | Admin/Ops |
...
- Landing pre-registration is public and does not create a user, order, entitlement, subject, or Voice Persona application. `POST /api/v1/pre-registrations` accepts `early_access`, `survey_10`, or `interview_20`; survey and interview selections require their matching conditional answer object, and contact consent is mandatory. The `survey_10` object has seven required answers: `firstSituation`, `recordingAvailability`, `recordSearchExperience`, `voiceLossRegret`, `desiredFeatures`, `voicePersonaFeeling`, and `concerns`. The optional `Idempotency-Key` header supports safe retries. A normalized email or Korean mobile number can have one active pre-registration. `GET` and `PATCH /api/v1/admin/pre-registrations` are restricted to Admin/Ops because they expose contact data; each read and status update writes a sanitized audit record without contact content.
